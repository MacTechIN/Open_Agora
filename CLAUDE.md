...
| `web/scripts/check-no-draft-leak.mjs` | 작성 중인 글이 기기 밖으로 나가는 것 |
| `web/scripts/check-screening.mjs` | 톤 판정이 코어와 갈리는 것 |
| `services/bridging/test_bridging.py` | **브리징이 인기투표로 퇴화하는 것 (★★)** |
...
* **톤 스크리닝 어휘에 정치 용어를 넣지 않습니다.** 정당·정치인·이념을
  가리키는 말이 들어가는 순간 이 플랫폼은 막으려던 바로 그 일을 하게 됩니다.
  코어와 웹 양쪽에 확인하는 시험이 있습니다.
* **점수는 아무것도 막지 않습니다.** 사용자는 언제나 원문을 게시할 수 있습니다.
  점수가 게시를 막는 순간 코치가 아니라 검열입니다.
...
`r̂(u,i) = μ + b_u + b_i + f_u · f_i`에서 **같은 진영의 몰표는 `f_u · f_i` 항이 흡수하므로 `b_i`를 올리지 못합니다.** 반대 성향 집단까지 긍정 평가해야만 `b_i`가 상승합니다.

**이 성질은 저절로 되지 않습니다.** 절편(`b_u`, `b_i`)을 인자(`f_u`, `f_i`)보다 10배 강하게 정규화해야 합니다. 한 진영만 반응하면 그 사람들의 `f_u`가 거의 상수라 설계행렬이 공선성을 갖고, 릿지가 노름이 큰 절편으로 설명을 몰아주기 때문입니다. 명세대로 λ 하나만 쓰면 **몰표가 브리징을 이깁니다** (→ D23).

랭킹·반응·평판 코드를 건드리면 `services/bridging/test_bridging.py`가 반드시 통과해야 합니다. CI에 영구 등록되어 있습니다.
