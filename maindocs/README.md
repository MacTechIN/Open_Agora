# CivicAgora 통합 명세서 (maindocs)

이 폴더는 **단일 진실 공급원(Single Source of Truth)** 입니다. 기존 `docs/` 폴더의 문서들은 서로 상충하는 내용을 담고 있어 **아카이브(참고용, 비권위)** 로 강등되었습니다. 구현 시 상충이 발견되면 언제나 `maindocs/`가 우선합니다.
...
처음 합류한다면 `00` → `02` → `01` → `03` 순서를 권합니다. 기존 `docs/`를 이미 읽었다면 **`08_DECISIONS.md`를 먼저** 읽으십시오. 무엇이 왜 바뀌었는지가 모두 거기에 있습니다.
